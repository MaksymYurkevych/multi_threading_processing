...
from pretty_tables import SortView
...
def sort_files(my_path):
    ...
    pt = SortView()
    result = ["\n".join(k for k in known_extensions), "\n".join(u for u in unknown_extensions)]
    print(pt.create_row(result))
...
def main():
    ...
    while True:
        ...
        if user_input not in sorted_folders:
